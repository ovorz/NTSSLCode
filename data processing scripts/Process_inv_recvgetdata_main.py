...
import os
RPC_USER = 'btc_tiancy_fengfeng_@@'
RPC_PASSWORD = '123456_tiancy_fengfeng_@@'
...
def getMainnetHash(txid):
    ...
    return re['data']
file_handler = open('../Data_MultiNode_Mainnet/pzy/100%/MidResult/Self_Txs_pzy_0221_Pt2.txt', mode='r')
# 2.读取文件内容
...
inv_dst_ips = []
rs = os.path.exists('../Data_MultiNode_Mainnet/pzy/100%/inv_dst_ips/mainnet_pzy_inv_dst_ips0221_Pt2.txt')
if rs:
    # 1.打开文件
    file_handler = open('../Data_MultiNode_Mainnet/pzy/100%/inv_dst_ips/mainnet_pzy_inv_dst_ips0221_Pt2.txt', mode='r')
    # 2.读取文件内容
    contents = file_handler.readlines()
    for msg in contents:
        msg = msg.strip('\n')  # 去除字符串中的\n
    ...
    file_handler.close()
partation_link = random.sample(inv_dst_ips, int(len(inv_dst_ips)))
# np.savetxt("../Data_MultiNode_Mainnet/tcy/25%/inv_dst_ips/mainnet_tcy_inv_dst_ips1208_Pt2_25%.txt", np.array(partation_link), fmt='%s',delimiter=',')
...
for block in blocks:  # 每一个block表示一个包
    if block['_source']['layers']['ip']['ip.dst'] == pzy_ip and 'bitcoin' in block['_source']['layers'].keys() and block['_source']['layers']['ip']['ip.src'] in partation_link:
...
print(len(invs))
for inv in invs:
    if inv['_source']['layers']['ip']['ip.src'] == pzy_ip and 'bitcoin' in inv['_source']['layers'].keys() and inv['_source']['layers']['ip']['ip.dst'] in partation_link :
...
if __name__ == '__main__':
    ...
    data = np.array(data1)
    np.savetxt("../Data_MultiNode_Mainnet/pzy/100%/TestSet/train_y_inv&recv_get_pzy_0221_100%_Pt2.txt", lable, fmt='%f',
               delimiter=',')
    np.savetxt("../Data_MultiNode_Mainnet/pzy/100%/TestSet/main_inv&recv_get_pzy_0221_100%_Pt2.txt", data, fmt='%f', delimiter=',')
    print("list:", len(data1))
    info1 = np.array(info)
    np.savetxt("../Data_MultiNode_Mainnet/pzy/100%/TestSet/main_tx_inv&recv_get_pzy_0221_100%_Pt2.txt", info1, fmt='%s',
               delimiter=',')
